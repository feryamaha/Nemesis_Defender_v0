...
O problema que motiva o projeto é real e contínuo. Só em 2025 a Sonatype identificou mais de 454 mil novos pacotes maliciosos, elevando o total acumulado a mais de 1,2 milhão em npm, PyPI, Maven, NuGet e Hugging Face, e 2026 segue no mesmo ritmo, com campanhas como Shai-Hulud/Mini Shai-Hulud e o grupo TeamPCP comprometendo pacotes legítimos (TanStack, PyTorch Lightning, LiteLLM) em waves coordenadas. Some-se a isso um padrão que desenvolvedores vêm relatando no X, no LinkedIn e no Reddit: agentes LLM que, mesmo instruídos a não apagar nem destruir nada, em tarefas rotineiras decidiram por conta própria, sem que o humano percebesse, e executaram deleções de arquivos. Casos públicos confirmam o padrão: o agente do Replit apagou um banco de produção durante um code freeze ativo, apesar de instruções repetidas (AI Incident Database #1152), e o Gemini CLI destruiu arquivos de um projeto após interpretar mal uma sequência de comandos (#1178). O Nemesis ataca a janela em que esse código tenta executar na máquina do desenvolvedor.
...
**Fase 1, Regras em markdown.** No início, eram apenas convenções escritas, dentro do ambiente da Windsurf (hoje transicionada para Devin Desktop, da Cognition), onde o Nemesis nasceu nativamente, tentando conter os mesmos erros que apareciam sem parar: hooks condicionais, `setState` síncrono dentro de `useEffect`, uso de `any`, tipagens inline duplicadas, CSS manual fora do `tailwind.config`, lógica de negócio embutida em arquivos de UI. Esses anti-patterns surgiam independentemente da experiência do desenvolvedor humano ou da capacidade do modelo de IA. A lição que ficou: regra escrita é *input de contexto*, o modelo lê, entende o conceito abstrato, e ainda assim executa o padrão neural de "resolver rápido". Instrução não é enforcement.
...
- **eBPF / kernel (adicional, Linux)**, `violations.log` registra bloqueios com `"layer":"ebpf"`, `"type":"permission_denied"`, cobrindo `rm`, `shred`, `dd`, `truncate`, `kill`, `chmod`, `mount`, `nc`, e a execução de runtimes arbitrários (`python3`, `perl`). São bloqueios no kernel, não dependentes de deny-list em userspace. Os volumes de bloqueio eBPF/malicious registrados vêm de testes, pentests, validação e calibração periódica do código-fonte do Nemesis, não de produção.
- **Qualidade (junto com o pretool)**, os mais de 2.000 bloqueios reais acumulados em produção são da camada de qualidade: tentativas de tipagem fraca (`any`), aninhamento de âncoras/tags que quebra build, hooks condicionais dentro de `if/else` e problemas gerais dessa natureza, barrados no momento da escrita.
...
A biblioteca Rust (`nemesis-defender`) é agnóstica de IDE. Cada IDE/TUI contribui hooks que invocam os binários: Claude Code, Codex, Cursor, VS Code (via pretool), Windsurf (hoje Devin Desktop, da Cognition) e OpenClaude.
