...
pub fn visit_js_node(node: &Node, source: &str) -> Vec<DefenderViolation> {
    ...
    if node.kind() == "call_expression" {
        if (node_text.contains("fetch") || node_text.contains("axios")) && node_text.contains("eval") {
        ...
        if node_text.contains("require") && (node_text.contains("http://") || node_text.contains("https://")) {
            violations.push(DefenderViolation {
                visitor: "url_in_exec".to_string(),
                line: (node.start_position().row + 1) as u32,
                col: (node.start_position().column + 1) as u32,
                ...
                message: "require() called with HTTP URL. \
                         Remote code execution: module loaded from untrusted URL.".to_string(),
                suggestion: Some(SUGGESTION_URL_EXEC.to_string()),
            });
        }

        if node_text.contains("import(") && (node_text.contains("http://") || node_text.contains("https://")) {
            violations.push(DefenderViolation {
                visitor: "url_in_exec".to_string(),
                line: (node.start_position().row + 1) as u32,
                col: (node.start_position().column + 1) as u32,
                ...
                message: "Dynamic import() with HTTP URL. \
                         Remote code execution: module dynamically loaded from untrusted URL.".to_string(),
                suggestion: Some(SUGGESTION_URL_EXEC.to_string()),
            });
        }
    }
        ...
        if node_text.contains("bash") || node_text.contains("sh") || node_text.contains("eval") {
            violations.push(DefenderViolation {
                ...
                message: "curl/wget piped to shell execution. \
                         Classic download-and-execute malware pattern.".to_string(),
                suggestion: Some(SUGGESTION_URL_EXEC.to_string()),
            });
        }
    }
    ...
    if (node_text.contains("urllib") || node_text.contains("requests"))
        ...
        violations.push(DefenderViolation {
            ...
            message: "HTTP request followed by exec/eval. \
                     Python download-and-execute pattern.".to_string(),
            suggestion: Some(SUGGESTION_URL_EXEC.to_string()),
        });
    }

    if node_text.contains("exec") && (node_text.contains("http://") || node_text.contains("https://")) {
        violations.push(DefenderViolation {
            visitor: "url_in_exec".to_string(),
            line: (node.start_position().row + 1) as u32,
            col: (node.start_position().column + 1) as u32,
            ...
            message: "exec() called with HTTP URL argument. \
                     Remote code execution via URL parameter.".to_string(),
            suggestion: Some(SUGGESTION_URL_EXEC.to_string()),
        });
    }
