...
pub fn log_result(result: &DefenderResult) -> std::io::Result<()> {
    ...
    writeln!(file, "\n{} {} {}", timestamp, severity_tag, result.path.display())?;
    writeln!(file, "  Language: {:?} | Depth: {}", result.language, result.scan_depth)?;

    for v in &result.violations {
        writeln!(file, "  ├─ [{}] Line {}:{} — {}", v.visitor, v.line, v.col, v.message)?;
