...
fn check_new_expression(node: &tree_sitter::Node, source: &str, violations: &mut Vec<Violation>) {
    ...
    if is_async_function(first, source) {
        ...
        ).with_suggestion("[STOP] Leia .windsurf/rules/typescript-typing-convention.md antes de reescrever. Nao use async em executores de Promise. Erros nao sao capturados. Consulte: https://biomejs.dev/"));
    }
}
