...
fn visit_node(cursor: &mut tree_sitter::TreeCursor, source: &str, violations: &mut Vec<Violation>) {
    ...
    if node.kind() == "string" {
        ...
        if text.contains("${") {
            ...
            ).with_suggestion("[STOP] Leia .windsurf/rules/typescript-typing-convention.md antes de reescrever. Use template literal: `valor ${var}`. Consulte: https://biomejs.dev/").with_severity(Severity::Warning));
        }
    }
