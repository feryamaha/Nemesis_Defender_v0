...
fn visit_node(cursor: &mut tree_sitter::TreeCursor, source: &str, violations: &mut Vec<Violation>) {
        ...
        || node.kind() == "break_statement" || node.kind() == "continue_statement"
    {
        if is_inside_finally(&node) {
            ...
            ).with_suggestion("[STOP] Leia .windsurf/rules/typescript-typing-convention.md antes de reescrever. Evite controle de fluxo em finally. Isso suprime excecoes do try. Consulte: https://biomejs.dev/"));
        }
    }
